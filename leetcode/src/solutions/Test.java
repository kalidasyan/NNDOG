package solutions;

public class Test {
    
    public static void main(String[] args) {
//    	new HappyNumber().test(7);
//    	new NumberOf1Bits().test((int)2147483648L);
//    	new ReverseInteger().test(1534236469);
//    	new CompareVersionNumbers().test("1.0", "1.10");
//    	new ZigZagConversion().test("PAYPALISHIRING", 3);
//    	new MinStack().test();
//    	new MaximumProductSubarray().test();
//    	new ReverseWords().test();
//    	new WordSearch().test();
//    	new ValidSudoku().test();
//    	new WordSearch().test();
//    	new WordLadderII().test();
//    	new NumberOfDigitOne().test();
//    	new CombinationSumIII().test();
//    	new SearchA2DMatrixII().test();
//    	new CourseSchedule().test();
//    	new WordDictionary().test();
//    	new WordSearchII().test();
//    	new PalindromePartitioningII().test();
//    	new SurroundedRegionsBFS().test();
//    	new LongestPalindromicSubstring().test();
//    	new ShortestPalindrome().test();
//    	new LongestPalindromicSubstringManacher().test();
//    	new ValidNumber().test();
//    	new RotateList().test();
//    	new SearchForARange().test();
//    	new SearchInsertPosition().test();
//    	new RemoveDuplicatesFromSortedArrayII().test();
//    	new FractionToRecurringDecimal().test();
//    	new Anagrams().test();
//    	new SimplifyPath().test();
//    	new LongestSubstringWithoutRepeatingCharacters().test();
//    	new RegularExpressionMatching().test();
//    	new DivideTwoIntegers().test();
//    	new PowXN().test();
//    	new SqrtX().test();
//    	new FlattenBinaryTreeToLinkedList().test();
//    	new ConvertSortedArrayToBinarySearchTree().test();
//    	new ConvertSortedListToBinarySearchTree().test();
//    	new UniqueBinarySearchTreesII().test();
//    	new InvertBinaryTree().test();
//    	new InsertInterval().test();
//    	new MergeIntervals().test();
//    	new NextPermutation().test();
    	new PermutationSequence().test();
    }
}
